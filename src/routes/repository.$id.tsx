...
import {
  SearchPanel,
  type SearchResult,
} from "@/components/workspace/repository/panels/SearchPanel";
    ...
    switch (section) {
      case "overview":
        return (
          <OverviewPanel
            repo={repo}
            onFocusNode={(label) => setSelection({ kind: "node", label })}
          />
        );
      case "explorer":
        return (
      ...
      case "tech":
        return (
          <TechStackPanel onSelect={(label, why) => setSelection({ kind: "tech", label, why })} />
        );
      case "docs":
  ...
  const renderRight = () => {
          ...
          {n.files && (
            <PanelBlock label="Files">
              <ul className="space-y-0.5">
                {n.files.map((f) => (
                  <li key={f} className="font-mono text-[12.5px] text-mulberry/80">
                    {f}
                  </li>
                ))}
              </ul>
            ...
            <PanelBlock label="Used by">
              <div className="flex flex-wrap gap-1.5">
                {n.usedBy.map((u) => (
                  <span
                    key={u}
                    className="px-2 py-0.5 rounded-full bg-oxblood/[0.06] border border-oxblood/10 text-[12px] text-oxblood"
                  >
                    {u}
                  </span>
                ))}
          ...
          <PanelBlock label="Connected to">
            <div className="flex flex-wrap gap-1.5">
              {n.connections.map((c) => (
                <span
                  key={c}
                  className="px-2 py-0.5 rounded-full bg-oxblood/[0.06] border border-oxblood/10 text-[12px] text-oxblood capitalize"
                >
                  {c}
                </span>
              ))}
    ...
    if (selection.kind === "dep") {
      return (
        <RightPanel keyId={`dep-${selection.label}`} eyebrow="Dependency" title={selection.label}>
          <PanelBlock label="Role">
            Foundational to this project's runtime behaviour. Traced through the dependency chain to
            leaves.
          </PanelBlock>
          <PanelBlock label="Recommendation">
            Read this package's README before diving into files that import it.
          </PanelBlock>
        </RightPanel>
      );
    }
      ...
      return (
        <RightPanel keyId={`node-${selection.label}`} eyebrow="Layer" title={selection.label}>
          <PanelBlock label="Purpose">
            Part of {repo.name}'s project map — a layer in the reading order.
          </PanelBlock>
          <PanelBlock label="Suggested next step">
            Open Architecture to see how this layer connects to its neighbours.
          </PanelBlock>
        </RightPanel>
      );
    }
    return (
      <RightPanel keyId="default" eyebrow="Intelligence" title="Repository Summary">
        <PanelBlock label="Purpose">{repo.description}</PanelBlock>
        ...
        <PanelBlock label="Architecture">
          {repo.architecture}. State is co-located with screens; anything shared is lifted into a
          hook or a service.
        </PanelBlock>
