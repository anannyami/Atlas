...
export const Route = createFileRoute("/explore")({
  head: () => ({
    meta: [
      { title: "Explore — Atlas" },
      { name: "description", content: "Choose a repository to explore in Atlas. Curated codebases or analyze your own GitHub project." },
  ...
  return (
    <div className="pointer-events-none fixed inset-0 -z-10 atlas-stage atlas-noise">
      <motion.div
        ...
        style={{ width: 900, height: 900, top: "-25%", left: "-15%", background: "radial-gradient(circle, #F0E6B1, transparent 60%)" }}
        animate={{ x: [0, 60, -20, 0], y: [0, 40, -30, 0] }}
        transition={{ duration: 30, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="atlas-blob"
        style={{ width: 800, height: 800, top: "20%", right: "-15%", background: "radial-gradient(circle, #C7886B, transparent 65%)", opacity: 0.5 }}
        animate={{ x: [0, -50, 30, 0], y: [0, 60, 20, 0] }}
        transition={{ duration: 34, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="atlas-blob"
        style={{ width: 900, height: 900, bottom: "-30%", left: "30%", background: "radial-gradient(circle, #8E3E4D, transparent 60%)", opacity: 0.35 }}
...
function ExplorePage() {
  ...
  const filtered = useMemo(() => {
    ...
    return featuredRepositories.filter((r) => {
      ...
      if (!q) return true;
      const hay = [r.name, r.owner, r.description, r.framework, r.language, ...r.technologies].join(" ").toLowerCase();
      ...
      <main className="pt-28 pb-32">
        ...
        <section className="max-w-6xl mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <p className="mt-4 text-[15px] text-mulberry/75 max-w-xl">
              Start with one of our curated repositories or analyze your own GitHub project. Atlas will lay out its
              architecture so you can read with intent.
            </p>
          </motion.div>
        </section>
        ...
        <section className="max-w-6xl mx-auto px-6">
          <div className="flex items-end justify-between gap-4 mb-5">
            <div>
              <div className="text-[10.5px] font-mono uppercase tracking-wider text-mulberry/60">Section 01</div>
              <h2 className="font-serif text-3xl text-oxblood mt-1">Featured repositories</h2>
            </div>
            <div className="text-[12px] text-mulberry/60 hidden md:block">{filtered.length} of {featuredRepositories.length}</div>
          </div>

          <div className="mb-6">
            <RepositorySearch value={query} onChange={setQuery} activeFilter={filter} onFilter={setFilter} />
          </div>
        ...
        <section className="max-w-6xl mx-auto px-6 mt-24">
          <div className="mb-5">
            <div className="text-[10.5px] font-mono uppercase tracking-wider text-mulberry/60">Section 03</div>
        ...
        <section className="max-w-4xl mx-auto px-6 mt-24">
          <div className="mb-5">
            <div className="text-[10.5px] font-mono uppercase tracking-wider text-mulberry/60">Section 04</div>
  ...
  return (
    <div className="acrylic p-10 flex flex-col items-center text-center">
      <svg width="88" height="88" viewBox="0 0 88 88" fill="none">
        ...
        <circle cx="44" cy="44" r="34" stroke="url(#empty-g)" strokeWidth="1.5" fill="none" opacity="0.6" />
        <path d="M44 20 C 56 34, 56 54, 44 68 M44 20 C 32 34, 32 54, 44 68 M14 44 H74" stroke="url(#empty-g)" strokeWidth="1.2" fill="none" opacity="0.7" />
